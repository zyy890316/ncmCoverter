...
class neteaseMusic(object):
    ...
    def modified_id3(self, file_name, info):
        ...
        with http.request('GET', info['album_pic_url'], preload_content=False) as r, open('img.jpg', 'wb') as out_file:       
    ...
    def download_id3(self, amount_songs=None, n=None):
        '''
        下载歌曲ID3
        :amount_songs 下载几首
        :n 下第几首
        ...
        #dir_ = os.path.join(os.getcwd(), 'id3')
        dir_ = "/Users/yiyiz/Downloads" 

        for i in self.song_infos:
            for file in os.listdir(dir_):
                filename = os.fsdecode(file);
                filename_without_extension = os.path.splitext(filename)[0]
                if i['song_name'].lower() == filename_without_extension.lower():
                    print(s % (2, 97, u'\n  >> updating id3 for ' + filename_without_extension + u' '))
                    self.modified_id3(os.path.join(dir_, filename), i)
            time.sleep(0)
